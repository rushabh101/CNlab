...
void *server_handler(void *serverhandler)
{
    struct server_data *data = (struct server_data *)server_handler;
    char buffer[1024];

    // printf("Thread %d started\n", clients[2]);
    int sender, rcver;
    printf("yoyo: %d, %d\n", *data->sender, *data->rcver);
    while (1)
    {
...
int main(int argc, char *argv[])
{
    ...
    case 's':
        ...
        int ids[2] = {-1, -1};
        char *usrthread[2];
        struct server_data thread_data1;
        thread_data1.sender = &conns[0];
        thread_data1.rcver = &conns[1];
        usrthread[0] = malloc(256 * sizeof(char));
        thread_data1.username = usrthread[0];
        thread_data1.id = &ids[0];

        struct server_data thread_data2;
        thread_data2.sender = &conns[1];
        thread_data2.rcver = &conns[0];
        usrthread[1] = malloc(256 * sizeof(char));
        thread_data2.username = usrthread[1];
        thread_data2.id = &ids[1];
        ...
        if (pthread_create(&thread1, NULL, server_handler, &thread_data2) != 0)
        {
            perror("thread creation failed");
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < 5; i++)
        {
            connfd[i] = accept(sockfd, (struct sockaddr *)&cliaddr1, &len);
            printf("Connected to client1\n");

            recv(connfd[i], buffer, 1024, 0);
            strncpy(username[i], &buffer[5], 256);
            active_clients[i] = 1;

            strcpy(buffer, "200 OK\n");
            send(connfd[i], buffer, 1024, 0);

            if (i == 0)
                continue;

            // When someone leaves, update pointers to thread
            int temp = 0;
            while (temp != 2)
            {
                for (int j = 0; j < 5; j++)
                {
                    temp += active_clients[j];
                }
            ...
            for (int j = 0; j < 5; j++)
            {
                if (active_clients[j] != 0)
                {
                    conns[cnt] = connfd[j];
                    ids[cnt] = j;
                    strncpy(usrthread[cnt], username[j], 256);
                    cnt++;
                }
            }
        }
    ...
    case 'c':
        ...
        char usernamec[256];
        fgets(usernamec, 256, stdin);
        sprintf(buffer, "HELO %s", usernamec);
        send(sockid, buffer, 1024, 0);
        recv(sockid, buffer, 1024, 0);
        printf("%s\n", buffer);
